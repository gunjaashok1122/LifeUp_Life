...
Once you reset your password, log in with it and the app will automatically migrate your account so future resets are 100% in-app via OTP!`);
  ...
  return (
    <div className="min-h-full w-full flex flex-col items-center justify-center py-2 px-3 relative overflow-hidden bg-[#070b13]">
      ...
      {/* Main Container */}
      <div className="w-full max-w-md p-4 rounded-3xl border border-rpg-border/30 bg-rpg-card/40 backdrop-blur-xl shadow-glass flex flex-col relative z-10">
        
        {/* App Title & Logo */}
        <div className="flex flex-col items-center mb-2">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-rpg-xp to-rpg-level flex items-center justify-center shadow-lg shadow-rpg-level/20 mb-1.5 animate-float">
            <Shield className="w-5 h-5 text-white stroke-[2]" />
          </div>
          <h1 className="text-[19px] font-extrabold tracking-tight text-white font-sans bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">
            LevelUp Life
          </h1>
          <p className="text-[9px] font-bold text-rpg-discipline uppercase tracking-wider mt-0.5 flex items-center gap-1 whitespace-nowrap">
            <Sparkles className="w-2.5 h-2.5" /> Gamify Your Discipline OS
          </p>
        </div>

        {/* Error Alert Box */}
        {error && (
          <div className="p-3.5 mb-2.5 rounded-xl bg-red-950/30 border border-red-500/30 text-red-400 text-xs font-bold leading-relaxed transition-all">
        ...
        {showForgot ? (
          <div>
            {forgotStep === 'input' && (
              <form onSubmit={handleForgotSubmitIdentifier} className="space-y-4">
                <h2 className="text-lg font-bold text-white mb-0.5 animate-pulse">Recover Hero Account</h2>
                <p className="text-xs text-slate-400 leading-relaxed mb-3">
                  Enter your email, username, or phone number and we'll send a simulated recovery code to retrieve your account.
                </p>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Email, Username, or Phone</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-11 pr-4 py-2 rounded-xl bg-slate-950/50 border border-rpg-border/60 text-white placeholder-slate-600 text-sm focus:outline-none focus:border-rpg-xp focus:ring-1 focus:ring-rpg-xp/40 transition-all"
                ...
                <button
                  type="submit"
                  disabled={isWorking}
                  className="w-full py-2.5 rounded-xl bg-gradient-to-r from-rpg-xp to-blue-600 text-white font-bold text-sm shadow-lg shadow-rpg-xp/25 hover:opacity-90 hover:scale-[1.01] active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:pointer-events-none"
                >
                  {isWorking ? (
                    <>
                ...
                <button
                  ...
                  disabled={isWorking}
                  className="w-full py-2 rounded-xl border border-rpg-border/40 hover:bg-slate-900 text-slate-400 text-xs font-bold transition-all disabled:opacity-50"
                >
                  Back to Login
                </button>
              </form>
            ...
            {forgotStep === 'otp' && (
              <form onSubmit={handleForgotVerifyOtp} className="space-y-4">
                <h2 className="text-lg font-bold text-white mb-0.5">Verify Recovery Code</h2>
                <p className="text-xs text-slate-400 leading-relaxed mb-3">
                ...
                <div className="flex justify-between gap-2">
                  {forgotEnteredOtp.map((digit, idx) => (
                    <input
                      ...
                      className="w-11 h-10 rounded-xl bg-slate-950/80 border border-rpg-border/50 text-white font-black text-lg text-center focus:outline-none focus:border-rpg-xp focus:ring-1 focus:ring-rpg-xp/30 transition-all"
                    />
                  ))}
                </div>
                ...
                <div className="flex gap-3 pt-1">
                  <button
                    ...
                    className="flex-1 py-2 rounded-xl border border-rpg-border/40 text-slate-400 text-xs font-bold hover:bg-slate-900 transition-all"
                  >
                    Back
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-2 rounded-xl bg-gradient-to-r from-rpg-xp to-rpg-level text-white text-xs font-bold shadow-lg shadow-rpg-level/25 hover:opacity-90 active:scale-95 transition-all"
                  >
                    Verify Code
                  </button>
                </div>
            ...
            {forgotStep === 'new_password' && (
              <form onSubmit={handleForgotSetNewPassword} className="space-y-3">
                <h2 className="text-lg font-bold text-white mb-0.5">Set New Password</h2>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      placeholder="••••••••"
                      className="w-full pl-11 pr-11 py-2 rounded-xl bg-slate-950/50 border border-rpg-border/60 text-white placeholder-slate-600 text-sm focus:outline-none focus:border-rpg-xp focus:ring-1 focus:ring-rpg-xp/40 transition-all"
                ...
                <button
                  type="submit"
                  disabled={isWorking}
                  className="w-full py-2.5 mt-1.5 rounded-xl bg-gradient-to-r from-rpg-xp to-rpg-level text-white font-bold text-sm shadow-lg shadow-rpg-level/25 hover:opacity-90 hover:scale-[1.01] active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:pointer-events-none"
                >
                  {isWorking ? (
                    <>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            {/* Tabs */}
            <div className="flex p-1 bg-slate-950/60 rounded-xl border border-rpg-border/40 mb-1">
              <button
                type="button"
              ...
              <>
                ...
                <div>
                  ...
                  {usernameStatus === 'checking' && (
                    <span className="text-[10px] font-semibold text-rpg-level mt-0.5 block">
            ...
            {isLogin && (
              <button
                ...
                disabled={isWorking}
                className="w-full py-2.5 mt-2 rounded-xl border border-rpg-border/60 hover:bg-slate-900/40 text-slate-300 font-bold text-sm transition-all flex items-center justify-center gap-2 disabled:opacity-50 active:scale-95"
              >
                CREATE ACCOUNT
              </button>
            )}
        ...
        <button
          ...
          disabled={isWorking}
          className="w-full py-2 rounded-xl bg-white text-slate-900 hover:bg-slate-100 text-xs font-bold transition-all flex items-center justify-center gap-2 disabled:opacity-50 active:scale-95"
        >
