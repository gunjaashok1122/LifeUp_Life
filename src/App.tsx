...
const LayoutWrapper: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#070b13] text-slate-200">
      
      {/* --- DESKTOP SIDEBAR --- */}
      <aside className="hidden md:flex md:w-64 flex-col bg-slate-950/70 border-r border-rpg-border/30 p-5 space-y-8 select-none flex-shrink-0 z-30">
        <div>
          <h1 className="text-xl font-black tracking-tight text-white flex items-center gap-2">
            <span className="p-1.5 rounded-lg bg-gradient-to-tr from-rpg-xp to-rpg-level">
              🛡️
            </span>
            LevelUp Life
          </h1>
          <p className="text-[9px] font-bold text-rpg-discipline uppercase tracking-widest mt-1 ml-9">
            Discipline OS
          </p>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3.5 px-4.5 py-3 rounded-xl text-xs font-bold transition-all relative ${
                  active 
                    ? 'bg-gradient-to-r from-rpg-xp/25 to-indigo-950/10 border-l-4 border-rpg-xp text-white' 
              ...
              >
                ...
                {isMessages && totalUnreads > 0 && (
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 min-w-5 h-5 px-1.5 flex items-center justify-center bg-red-500 text-white text-[9px] font-black rounded-full shadow-md animate-pulse">
                    {totalUnreads}
                  </span>
                )}
              </button>
        ...
        {/* Small character profile indicator in sidebar */}
        <div 
          ...
          className="p-3 rounded-2xl bg-slate-950 border border-rpg-border/40 flex items-center gap-3 cursor-pointer hover:border-rpg-border/70 transition-all select-none"
        >
          <AvatarBuilder config={user.avatar} size={36} showCamera={false} />
          <div className="truncate">
            <div className="text-xs font-bold text-white truncate">{user.name}</div>
            <div className="text-[9px] font-semibold text-slate-500">Hero</div>
          </div>
        </div>
      </aside>

      {/* --- MOBILE HEADER --- */}
      <header className="md:hidden flex items-center justify-between p-4 bg-slate-950/80 border-b border-rpg-border/20 backdrop-blur-md sticky top-0 z-40">
      ...
      {/* --- MAIN MAIN SCROLL CONTENT AREA --- */}
      <main className="flex-1 p-4 md:p-8 overflow-y-auto pb-24 md:pb-8 max-w-5xl mx-auto w-full">
        <ScreenRenderer />
      </main>

      {/* --- MOBILE BOTTOM NAVBAR --- */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-950/90 border-t border-rpg-border/30 backdrop-blur-lg flex justify-around py-2.5 px-1 select-none">
